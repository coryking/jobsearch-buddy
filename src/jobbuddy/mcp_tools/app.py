...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "touch bumps. When a watchlist is passed, its saved curation is "
        "composed on top of caller params (queries AND, companies "
        "intersect, exclusions union) — narrowing only, never loosened — "
        "while its saved recency window (`posted_since`/`published_since`) "
        "is a default the caller overrides freely, narrower or wider. So "
        "the curation is preserved but the caller always owns the time "
        "window. "
        ...
        "- search_jobs, survey_jobs_by_companies, and find_companies all accept "
        "an optional `ats` array to narrow by ATS platform; pass what the user "
        "said and we'll normalize the match.\n"
