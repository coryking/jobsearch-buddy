"""Eightfold AI v2 API fetcher.

Some Eightfold career sites (Netflix, Qualcomm, NetApp) use /api/apply/v2/
instead of the older /api/pcsx/ endpoints. Same data model, different paths
and response shape.
"""
...
PAGE_SIZE = 10  # Eightfold returns exactly 10 per page


class EightfoldV2Fetcher(ATSFetcher):
    ats_type = "eightfold_v2"
    ...
    def _require_config(self) -> None:
        if not self.base_url or not self.domain:
            raise ValueError(
                f"EightfoldV2 fetcher for '{self.board}' requires base_url and domain. "
                "These are normally set from the company registry."
            )

    def _api_url(self, path: str) -> str:
        return f"{self.base_url}/api/apply/v2/{path}"
    ...
    def list_jobs(
        ...
        *,
        ...
        def _fetch_page0():
            resp = self.client.get(self._api_url("jobs"), params={**base_params, "start": 0})
        ...
        def _fetch_page(start: int) -> list[Job]:
            def _do_fetch():
                page_resp = self.client.get(self._api_url("jobs"), params={**base_params, "start": start})
                page_resp.raise_for_status()
                return [self._position_to_job(pos) for pos in page_resp.json().get("positions", [])]
    ...
    def _fetch_detail(self, job_id: str) -> dict:
        """Fetch job detail from v2 API."""
        params = {"domain": self.domain}
        resp = self.client.get(self._api_url(f"jobs/{job_id}"), params=params)
        resp.raise_for_status()
        return resp.json()
    ...
    def fetch_description(self, job_id: str, metadata: dict | None = None) -> str | None:
        """Fetch description directly from v2 detail API."""
        self._require_config()
        data = self._fetch_detail(job_id)
