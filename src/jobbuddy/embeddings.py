...
caller is the EmbedPhase ThreadPoolExecutor (parallelism handled there) or
a one-shot query at search time. ~700 companies and one query embedding
...
def embed_text(text: str) -> tuple[str, int]:
    ...
    `%s::vector`. Tokens are surfaced so the EmbedPhase can update the
    display token counter; query-time callers can ignore them.
    """
    client = _get_client()
