...
Prompts for 4 dimension scores (1-5) per sample. Saves to CSV with resume support.
"""
...
def _score_sample(console: Console, filename: str, original: str, stripped: str, run_name: str) -> dict | Literal["skip"] | None:
    ...
    console.print("[bold]Score each dimension 1-5:[/bold]")
    console.print("  1=failed  2=significant problems  3=decent  4=minor issues  5=perfect")
    console.print("  (q to quit, s to skip)")
    console.print()

    scores = {}
    labels = {
        "recall": "Recall (did differentiating content survive?)",
        "precision": "Precision (was boilerplate removed?)",
        "integrity": "Integrity (any fabricated content?)",
        "fidelity": "Fidelity (kept text match original surface form?)",
    }
